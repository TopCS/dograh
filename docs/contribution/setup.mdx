...
Pull requests are automatically tested by GitHub Actions:

- **API tests** (`.github/workflows/api-tests.yml`) run the backend pytest suite when `api/**`, `pipecat/**`, or `scripts/**` change.
- **LiveKit bridge tests** (`.github/workflows/livekit-bridge-tests.yml`) run the `dograh-livekit` pytest suite (`client`, `flow`, `models`, `stages`, `tools`, `translator`) when `dograh-livekit/**` changes. The bridge tests use HTTP mocks (`respx`) and stub credentials — they never require a live LiveKit server, nor Google/OpenAI/Deepgram/Cartesia API keys.

Changes under `dograh-livekit/` trigger only the LiveKit bridge workflow — the API tests workflow's paths filter covers `api/**`, `pipecat/**`, and `scripts/**`, not the bridge — so the bridge job is the sole gate for bridge changes.
...
| `dograh-livekit/` | LiveKit agent bridge (OSS path) — self-contained, opt-in via the `livekit` compose profile |
...
### `extension "vector" is not available` — missing pgvector

Dograh stores knowledge-base embeddings in pgvector `vector` columns, and the migration `dc33eef8dabe_add_document_tables` enables the extension the first time it runs. If your local PostgreSQL server does not ship pgvector, `alembic upgrade head` — and the API test session, which runs migrations on `test_db` at startup — fails during setup:
...
This is a server-side install issue, not a code or Python-dependency problem. Follow the [pgvector installation guide](/contribution/install-pgvector) to add the extension for your PostgreSQL version (Homebrew, apt, Alpine, Docker, or a source build), then re-run the tests — the migration enables `vector` on `test_db` automatically, so no `DATABASE_URL` change is needed.

**Need an immediate workaround?** Point `DATABASE_URL` at any Postgres that already has pgvector — for example the `postgres` service in `docker-compose-local.yaml` (image `pgvector/pgvector:pg17`, port 5432, the same image CI uses) or any local `pgvector/pgvector` container:
