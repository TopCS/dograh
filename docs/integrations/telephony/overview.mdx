...
Dograh AI's telephony integration system provides a unified interface for connecting with various telephony providers. The same configuration powers both outbound calls (initiated from Dograh) and inbound calls (received on a phone number you own). Besides the built-in managed providers, Dograh ships the **LiveKit OSS bridge** — an open-source, self-hosted path powered by the LiveKit Agents framework.
...
<CardGroup cols={3}>
  ...
  <Card title="LiveKit OSS" href="/integrations/telephony/livekit">
    Open-source self-hosted bridge for SIP and Room inbound patterns
  </Card>
